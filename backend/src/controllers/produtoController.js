...
module.exports = {
    ...
    async select(req, res) {
        try {
            const { id } = req.body; // ou req.params, dependendo da sua rota
    ...
    async delete(req, res) {
        try {
            ...
            const resultado = await ProdutoRepo.delete(id);
            
            if (!resultado || resultado.deletedCount === 0) {
                return res.status(404).json({ erro: "Produto não encontrado para deletar." });
            }
