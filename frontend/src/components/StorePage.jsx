...
export const StorePage = () => {
  ...
  useEffect(() => {
    fetchMedications().then(data => {
      console.log(data)
      setMedications(data.produtos)
    });
    fetchNurses(); //
  }, [fetchNurses]);

  const lowStockMeds = medications.filter((med) => med.qnt_estoque <= 10);
    ...
    <div>
        ...
        <table className="table">
          ...
          <tbody>
            {medications.map((med) => (
              <tr key={med._id}>
                <td>
                  <div style={{ fontWeight: '500' }}>{med.nome}</div> <span>Marca: </span>
                  <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>{med.marca}</span>
                  {med.qnt_disp <= 10 && <span className="badge badge-danger">Baixo</span>}
                </td>
                <td>{med.qnt_estoque}</td>
                <td>
      ...
      {isOpen && (
        <div className="modal-overlay" onClick={() => { setIsOpen(false); resetForm(); }}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h2 className="modal-title">{editingMed ? 'Editar' : 'Novo'} Produto</h2></div>
      ...
      {dispenseModalOpen && (
        <div className="modal-overlay" onClick={() => setDispenseModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h2 className="modal-title">Dispensar Produto</h2></div>
            <div className="modal-body">
